...
import { integrationTest, withTmpDir, writeFiles } from "../test_utils.ts";
import {
  getStdOutput,
...
integrationTest("Builder - chain onTransformStaticFile", async () => {
...
integrationTest("Builder - handles Windows paths", async () => {
  await using _tmp = await withTmpDir();
  const tmp = _tmp.dir;
...
integrationTest("Builder - hashes CSS urls by default", async () => {
  await using _tmp = await withTmpDir();
  const tmp = _tmp.dir;
  const builder = new Builder({
    outDir: path.join(tmp, "dist"),
...
integrationTest("Builder - exclude files", async () => {
  await using _tmp = await withTmpDir();
  const tmp = _tmp.dir;
...
integrationTest(
  "Builder - workspace folder middleware on listen",
  async () => {
    await using _tmp = await withTmpDir();
    const tmp = _tmp.dir;
...
integrationTest("Builder - .well-known static files", async () => {
  await using _tmp = await withTmpDir();
  const tmp = _tmp.dir;
...
integrationTest("Builder - write prod routePattern", async () => {
...
integrationTest("Builder - file:// islands", async () => {
  const root = path.join(import.meta.dirname!, "..", "..");
  await using _tmp = await withTmpDir({ dir: root, prefix: "tmp_builder_" });
  const tmp = _tmp.dir;
...
integrationTest(
  "Builder - dev server doesn't overtake _error handler",
  async () => {
    const root = path.join(import.meta.dirname!, "..", "..");
    await using _tmp = await withTmpDir({
      dir: root,
...
integrationTest("Builder - dev server supports basePath", async () => {
  const root = path.join(import.meta.dirname!, "..", "..");
  await using _tmp = await withTmpDir({ dir: root, prefix: "tmp_builder_" });
  const tmp = _tmp.dir;
...
integrationTest("Builder - serves static files in subdir", async () => {
  const root = path.join(import.meta.dirname!, "..", "..");
  await using _tmp = await withTmpDir({ dir: root, prefix: "tmp_builder_" });
  const tmp = _tmp.dir;
...
integrationTest(
  "Builder - serves static files in subdir in prod",
  async () => {
    const root = path.join(import.meta.dirname!, "..", "..");
    await using _tmp = await withTmpDir({
      dir: root,
...
integrationTest(
  "Builder - dev server error_overlay ignores _app and _layout",
  async () => {
    const root = path.join(import.meta.dirname!, "..", "..");
    await using _tmp = await withTmpDir({
      dir: root,
...
integrationTest("Builder - source maps", async () => {
  const root = path.join(import.meta.dirname!, "..", "..");
  await using _tmp = await withTmpDir({ dir: root, prefix: "tmp_builder_" });
  const tmp = _tmp.dir;
...
integrationTest("Builder - compile entry", async () => {
  const root = path.join(import.meta.dirname!, "..", "..");
  await using _tmp = await withTmpDir({ dir: root, prefix: "tmp_builder_" });
...
integrationTest(
  "Builder - island named after global object (Map)",
  async () => {
    const root = path.join(import.meta.dirname!, "..", "..");
    await using _tmp = await withTmpDir({
      dir: root,
